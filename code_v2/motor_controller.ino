...
class MotorController {
  ...
  private:
    ...
    array<float, 4> getMotorSpeeds(float theta, float speed, float tilt, float goal_tilt) {
      ...
      if (goal_tilt < 0 and (tilt > goal_tilt - 5 and tilt < goal_tilt + 10)) {
        tilt = 0;
      }

      else if (goal_tilt > 0 and (tilt > goal_tilt -10 and tilt < goal_tilt + 5)) {
        tilt = 0;
      }
      ...
      float highestVal = find_max(motorRatio, 4);
      float lowestVal =  find_min(motorRatio, 4);
  ...
  public:
    ...
    void runMotors(float x = 0, float y = 0, float tilt = 0, float goal_tilt = 0) {
      ...
      motorSpeeds = getMotorSpeeds(theta, speed, tilt, goal_tilt);
