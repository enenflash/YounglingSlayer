...
String ultrasonic_side_chooser(int left_distance, int right_distance, std::vector<int> left_distance_history, std::vector<int> right_distance_history, int FIELD_WIDTH, int ULTRASONIC_TO_ROBOT) {
    ...
    if (sensor_blocked) {
      ...
      int common_left_distance = Most_Common(left_distance_history);
      int common_right_distance = Most_Common(right_distance_history);
...
// find true distance due to tilt
int Get_True_Distance(int distance, float triangle_tilt) {
  return cos(Deg_To_Rad(triangle_tilt)) * distance;
}
